...
let MUTATE_CONSTANT = STORED_CONSTANT ? Number(STORED_CONSTANT) : 1.5;
...
const smartCars = generateSmartCars(SMART_CARS_NUM);
const dummyCars = loadDummies();
function saveBestBrain() {
    ...
    if (!highScore || smartCars[0].score > Number(highScore)) {
        ...
        let failCount = Number(localStorage.getItem(STORAGE_FAIL));
        console.log("increasing mutate constant");
        let newConstant = Math.min(MAX_MUTATE, MUTATE_CONSTANT * ((1 + MUTATE_GROWTH) ** failCount));
...
function generateSmartCars(num) {
    let cars = [];
    for (let i = 0; i < num; i++) {
        ...
        let car = new Car(road.getLaneXval(2), START_Y, false, brain);
        cars.push(car);
    }
    return cars;
}
function generateDummyCars(num) {
    let cars = [];
...
function loadDummies() {
    let cars = [];
    const dummies = JSON.parse(localStorage.getItem(STORAGE_DUMMIES));
